...
void setup_test07tvw(void) {
    test07.file = (struct WRAPPER_FILE*)calloc(1,
            sizeof(struct WRAPPER_FILE));
    test07.err = read_wrapper("test/resources/test07.tvw",
            test07.file);
    cr_assert(test07.err == 0,
            "got nonzero return code from read_wrapper!");
}
void teardown_test07tvw(void) {
    tvwfree(test07.file);
}

TestSuite(tvwi_test07, .init = setup_test07tvw, .fini = teardown_test07tvw);

Test(tvwi_test07, fileops) {
    cr_expect(test07.file->fp == NULL,
            "wrapper file pointer is still open!");
}

Test(tvwi_test07, header) {
    cr_expect(test07.file->header.format_version == 0,
            "wrong format version!");
    cr_expect(test07.file->header.start_of_contents_gs == 820,
            "wrong SOC GS offset!");
    cr_expect(test07.file->header.len_of_contents == 213,
            "wrong contents length!");
    cr_expect(test07.file->header.comp_algo_meta == 0,
            "wrong metadata compression algo!");
    cr_expect(test07.file->header.comp_algo_file == 0,
    ...
    cr_expect_arr_eq(test07.file->header.sha512, good, 64,
            "wrong file contents hash!");
}

Test(tvwi_test07, metadata) {
    cr_expect(test07.file->xmlerr == NULL,
            "xmlerr is not NULL, parsing failed!");
    cr_assert(test07.file->xmlroot != NULL,
            "xmlroot is NULL, parsing *really* failed!");
    cr_expect(test07.file->sizeof_meta == 732,
            "did not read correct metadata size!");
    cr_assert(test07.file->metadata != NULL,
...
"   </tvm>\n\n";
    ...
    cr_expect_str_eq(test07.file->metadata, good,
            "(raw) metadata is incorrect!");

}

Test(tvwi_test07, contents) {
    cr_expect(test07.file->contents != NULL,
            "file contents is NULL, something failed!");
    cr_expect(test07.file->sizeof_cont == 213,
    ...
    cr_expect_str_eq(test07.file->contents, good,
